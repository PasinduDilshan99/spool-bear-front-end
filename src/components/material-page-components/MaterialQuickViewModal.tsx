...
import {
  X, Star, Zap, Layers, Thermometer, Droplets,
  CheckCircle2, XCircle, ArrowRight, ChevronLeft,
  ChevronRight, Package, Ruler, Palette,
} from "lucide-react";
...
const strengthConfig = {
  High:   { color: "#10b981", bg: "rgba(16,185,129,0.10)",  bar: "#10b981", width: "90%" },
  Medium: { color: "#f59e0b", bg: "rgba(245,158,11,0.10)",  bar: "#f59e0b", width: "55%" },
  Low:    { color: "#ef4444", bg: "rgba(239,68,68,0.10)",   bar: "#ef4444", width: "25%" },
};

const flexConfig = {
  High:   { color: "#6366f1", bg: "rgba(99,102,241,0.10)",  bar: "#6366f1", width: "90%" },
  Medium: { color: "#0ea5e9", bg: "rgba(14,165,233,0.10)",  bar: "#0ea5e9", width: "55%" },
  Low:    { color: "#64748b", bg: "rgba(100,116,139,0.10)", bar: "#64748b", width: "20%" },
};
...
const MetricBar = ({
  ...
  <div className="space-y-1.5">
    <div className="flex items-center justify-between">
      <span className="text-[11px] font-bold uppercase tracking-widest text-[#B8ADA4]">{label}</span>
      <span
...
const StatTile = ({
  ...
  <div
    ...
    style={{
      background: accent ? "linear-gradient(135deg,#FF5000,#FF7A40)" : "#F7F5F2",
      border: accent ? "none" : "1px solid #EAE4DC",
    }}
  >
    <div className="flex items-center gap-1.5">
      <Icon size={12} strokeWidth={2.5} style={{ color: accent ? "rgba(255,255,255,0.8)" : "#B8ADA4" }} />
      <span className="text-[10px] font-bold uppercase tracking-widest" style={{ color: accent ? "rgba(255,255,255,0.75)" : "#B8ADA4" }}>
        {label}
      </span>
    </div>
    <span
      className="text-[15px] font-black leading-none"
      style={{ color: accent ? "#fff" : "#1C1714", fontFamily: "'Fraunces','Georgia',serif" }}
    >
      {value}
    </span>
  </div>
  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        style={{ background: "rgba(15,10,8,0.65)", backdropFilter: "blur(8px)" }}
        onClick={onClose}
      >
        <motion.div
          key="panel"
          ...
          style={{ boxShadow: "0 32px 80px rgba(0,0,0,0.22), 0 8px 24px rgba(0,0,0,0.1)" }}
        ...
        >
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <h3
                className="text-lg font-black text-[#1C1714]"
            ...
            <button
              ...
              title="Close"
            >
              <X size={18} strokeWidth={2.5} className="text-[#6B5F56]" />
            </button>
          </div>
          ...
          <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="relative h-64 rounded-2xl overflow-hidden bg-[#F7F5F2]">
                ...
                {images[activeImg] && !imgError ? (
                  ...
                  <div className="flex flex-col items-center justify-center h-full gap-3">
                    <Layers size={44} className="text-[#D6CEC6]" strokeWidth={1.2} />
                ...
                <div className="flex gap-2">
                  {images.slice(0, 5).map((src, idx) => (
                    <button
                      ...
                      style={{
                        border: activeImg === idx ? "2px solid #FF5000" : "2px solid transparent",
              ...
              {material.colors && material.colors.length > 0 && (
                <div className="p-4 rounded-2xl bg-[#F7F5F2] border border-[#EAE4DC]">
                  <div className="flex items-center gap-2 mb-3">
                    <Palette size={13} strokeWidth={2} className="text-[#B8ADA4]" />
            ...
            <div className="flex flex-col gap-5">
              ...
              <div className="grid grid-cols-2 gap-2">
                {/* <StatTile icon={Package}     label="Price"    value={`$${material.pricePerGram.toFixed(3)}/g`} accent /> */}
                <StatTile icon={Droplets}    label="Density"  value={`${material.density} g/cm³`} />
                <StatTile icon={Thermometer} label="Max Temp" value={material.temperatureResistance} />
                <StatTile icon={Ruler}       label="Layer"    value={`${material.minLayerHeight}–${material.maxLayerHeight} mm`} />
              </div>

              {/* Finish + Type */}
              <div className="flex gap-2">
                <div className="flex-1 px-3.5 py-3 rounded-2xl bg-[#F7F5F2] border border-[#EAE4DC]">
                  <div className="text-[10px] font-bold uppercase tracking-widest text-[#B8ADA4] mb-0.5">Finish</div>
                  <div className="text-sm font-bold text-[#3D3530]">{material.finish}</div>
                </div>
                {material.materialType?.name && (
                  <div className="flex-1 px-3.5 py-3 rounded-2xl bg-[#F7F5F2] border border-[#EAE4DC]">
                    <div className="text-[10px] font-bold uppercase tracking-widest text-[#B8ADA4] mb-0.5">Type</div>
                    <div className="text-sm font-bold text-[#3D3530]">{material.materialType.name}</div>
                  </div>
                )}
              </div>

              {/* Strength & Flexibility bars */}
              <div className="p-4 rounded-2xl bg-[#F7F5F2] border border-[#EAE4DC] space-y-4">
                <MetricBar
                  label="Strength"
                  level={strength.label}
              ...
              {material.properties?.length > 0 && (
                <div>
                  <div className="flex items-center gap-2 mb-3">
                    <Zap size={13} strokeWidth={2.5} className="text-[#FF5000]" />
                    <span className="text-[11px] font-black uppercase tracking-widest text-[#3D3530]">Properties</span>
                  </div>
                  <div className="grid grid-cols-2 gap-2">
                    {material.properties.map((prop, idx) => (
                      <div key={idx} className="px-3 py-2.5 rounded-xl bg-[#F7F5F2] border border-[#EAE4DC]">
                        <div className="text-[10px] text-[#B8ADA4] font-semibold mb-0.5">{prop.propertyName}</div>
                        <div className="text-[13px] font-bold text-[#3D3530]">{prop.propertyValue}</div>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="p-3.5 rounded-2xl bg-emerald-50 border border-emerald-100">
                    <div className="flex items-center gap-1.5 mb-2.5">
                      ...
                      <span className="text-[11px] font-black uppercase tracking-widest text-emerald-600">Pros</span>
                    </div>
                    <ul className="space-y-1.5">
                      {material.pros.map((pro, idx) => (
                        <li key={idx} className="flex items-start gap-1.5 text-[12px] text-emerald-800 leading-snug">
                  ...
                  <div className="p-3.5 rounded-2xl bg-red-50 border border-red-100">
                    <div className="flex items-center gap-1.5 mb-2.5">
                      ...
                      <span className="text-[11px] font-black uppercase tracking-widest text-red-500">Cons</span>
                    </div>
                    <ul className="space-y-1.5">
                      {material.cons.map((con, idx) => (
                        <li key={idx} className="flex items-start gap-1.5 text-[12px] text-red-800 leading-snug">
          ...
          >
            <div>
              {/* <div
                className="text-2xl font-black text-[#FF5000] leading-none"
                style={{ fontFamily: "'Fraunces','Georgia',serif" }}
              >
                ${material.pricePerGram.toFixed(3)}
              </div> */}
              {/* <div className="text-[10px] text-[#B8ADA4] font-semibold uppercase tracking-wide">per gram</div> */}
            </div>

            <div className="flex gap-2">
              <button
                onClick={onClose}
                className="px-5 py-2.5 rounded-2xl text-sm font-bold text-[#6B5F56] bg-[#F5F0EA] hover:bg-[#EAE4DC] transition-colors duration-150"
              >
                Close
              </button>
              <Link
              ...
              >
                Full Details
                <ArrowRight size={14} strokeWidth={2.5} className="transition-transform duration-200 group-hover/cta:translate-x-0.5" />
              </Link>
            </div>
          </div>
        </motion.div>
