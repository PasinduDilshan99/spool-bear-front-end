...
const PrintUploadForm: React.FC = () => {
  ...
  const handleSubmit = async () => {
    ...
    // Validation (customText is now optional)
    ...
    if (!formData.color.trim()) {
      return setStatus({ type: "error", message: "Please specify the color." });
    }
    ...
    try {
      ...
      const orderData: AddPrintingOrderRequest = {
        customText: formData.customText || "No title provided", // Default value if empty
                ...
                {formData.file ? (
                  <>
                    ...
                    <p className="text-xs text-gray-400">
                      {(formData.file.size / 1024 / 1024).toFixed(2)} MB · Click
                      to change
                    </p>
                  </>
                ) : (
